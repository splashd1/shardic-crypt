...
Readers most interested in the human-AI mutual-oversight case that motivates this paper's continued investment can skip ahead directly to §6.4–§6.5; §4 states the SPAC mechanics that pattern depends on, and requires nothing beyond what §1–§3 already establish.
...
3. **Ceremony formation** (§3.7, implemented). Turns trustee selection
   ...
   addressed event rather than an out-of-band handoff. Adopt it
   wherever a trustee pool is standing and reused across multiple
...
Every later component reuses the D-of-T guarantee from component 1
without modification — none of this is a rewrite of the underlying
math, only an extension of how a trustee's key is delivered, how a
...
system — component 4 is simply where this paper's remaining open work,
and its highest-value application, both live.
...
An operator with a file or directory to protect decides on two numbers before doing anything else: how many trustees (T) will each hold a shardic encryption key, and how many of them (D) must agree to recover the data. Given those numbers, vault creation is a fixed sequence: the input is archived into a single blob and encrypted once under a randomly generated 256-bit data encryption key (DEK); that DEK is split into T Shamir shards at threshold D; each shard is individually protected under its own shardic encryption key (§3.3); and the result is written as one self-contained `.krypt` file, plus one credential per trustee — whatever form that credential takes for the derivation source chosen (§3.3).
...
Without mask, the pool shards — even all of them — reconstruct only masked_secret, which is uniformly random and indistinguishable from noise without the mask to remove. Without at least pool_threshold pool shards, mask alone reveals nothing either. Both halves of the construction retain the same information-theoretic guarantee as the base scheme; layering them is what makes the prime trustee mathematically essential rather than merely conventionally required.
