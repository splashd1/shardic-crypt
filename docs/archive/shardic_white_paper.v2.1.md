...
**Status: design proposal.** shardic's protection can be extended beyond the demonstrated file protection and supporting shardic-envelope ceremony that has been fielded to deliver a much more critical level of protection at system level.This section, moves beyond the fielded capabiility of §1–§3. shardic Protected Action Code (SPAC) describes a proposed extension that is targeting the flexibility to be embeddable in a variety of clients. As such there is no existing implementation yet, though the target systems,concepts, and embodiment are under exploration. What is clear is that the SPAC capability and life-cycle elements will be consistent, though manifested in a variety of ways to address the client system security, performance, and CONOPS needs.
